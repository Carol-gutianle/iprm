...
class CETrainer(ABC):
    ...
    def __init__(
        ...
        self.celoss = nn.BCELoss(reduction='none')
        # Mixtral 8*7b
    ...
    def loss_fn(self,policy_chosen_logps, reference_chosen_logps,labels):
        logratio = policy_chosen_logps - reference_chosen_logps
        rewards = (self.beta * logratio).sigmoid()  # .detach()
        ...
        for epoch in range(start_epoch, self.epochs):
            ...
            for data in self.train_dataloader:
                if not self.packing_samples:
                    ...
                    chosen_logps, aux_loss, nll_loss = self.concatenated_forward(
                        self.model, chosen_ids, c_mask, prompt_id_lens
                    )
                    with torch.no_grad():
                        reference_chosen_logps, _, _ = self.concatenated_forward(
                            self.ref_model, chosen_ids, c_mask, prompt_id_lens
                        )
                else:
                    packed_input_ids, packed_attention_masks, packed_seq_lens, labels, prompt_id_lens = data
                    packed_input_ids, packed_attention_masks,labels = packed_input_ids.to(
                        torch.cuda.current_device()
                    ), packed_attention_masks.to(torch.cuda.current_device()),labels.to(torch.cuda.current_device())
                    chosen_logps, aux_loss, nll_loss = self.packed_samples_forward(
                        self.model, packed_input_ids, packed_attention_masks, packed_seq_lens, prompt_id_lens
                    )
                    with torch.no_grad():
                        reference_chosen_logps, _, _ = self.packed_samples_forward(
                            self.ref_model, packed_input_ids, packed_attention_masks, packed_seq_lens, prompt_id_lens
                        )

                # loss function
                preference_loss = self.loss_fn(
                    chosen_logps, reference_chosen_logps,labels
                )
                # mixtral
    ...
    def evaluate(self, eval_dataloader, steps=0):
        ...
        with torch.no_grad():
            ...
            for data in eval_dataloader:
                if not self.packing_samples:
                    chosen_ids, c_mask, prompt_id_lens = data
                    chosen_ids = chosen_ids.squeeze(1).to(torch.cuda.current_device())
                    c_mask = c_mask.squeeze(1).to(torch.cuda.current_device())
                    chosen_logps, aux_loss, _ = self.concatenated_forward(
                        self.model, chosen_ids, c_mask,  prompt_id_lens
                    )
                    with torch.no_grad():
                        reference_chosen_logps, _, _ = self.concatenated_forward(
    ...
    def concatenated_forward(self, model, input_ids, att_masks, prompt_id_lens):
        ...
        chosen_logps = all_logps_sum
        aux_loss = output.aux_loss if "aux_loss" in output else []
        return chosen_logps, aux_loss, -all_logps_mean.mean()
    ...
    def packed_samples_forward(self, model, packed_input_ids, packed_attention_masks, packed_seq_lens, prompt_id_lens):
        ...
        chosen_logps = all_logps_sum
        aux_loss = output.aux_loss if "aux_loss" in output else []
        return chosen_logps, aux_loss, -all_logps_mean[: len(packed_seq_lens) // 2].mean()
